<?php $__env->startSection('title', 'Shipping Rates'); ?>
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.common.page-breadcrumb','data' => ['pageTitle' => 'Shipping Rates']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['pageTitle' => 'Shipping Rates']); ?>
...
<div class="flex items-center justify-between mb-4">
    <div></div>
    <a href="<?php echo e(route('admin.shipping-rates.create')); ?>"
        ...
        <table class="w-full">
            <thead>
                <tr class="bg-gray-50 dark:bg-white/[0.02] border-b border-gray-200 dark:border-gray-800">
                    ...
                    <th class="px-4 py-3 text-left text-theme-xs font-medium text-gray-500 uppercase">Base Rate</th>
                    <th class="px-4 py-3 text-left text-theme-xs font-medium text-gray-500 uppercase">Free Threshold</th>
            ...
            <tbody>
                <?php $__empty_1 = true; $__currentLoopData = $shippingRates; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $shippingRate): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <tr class="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-white/5">
                        <td class="px-4 py-3.5 text-sm font-medium text-gray-800 dark:text-white/90"><?php echo e($shippingRate->country); ?></td>
                        <td class="px-4 py-3.5 text-sm text-gray-500 dark:text-gray-400"><?php echo e($shippingRate->region ?? '—'); ?></td>
                        <td class="px-4 py-3.5 text-sm text-gray-500 dark:text-gray-400"><?php echo e($shippingRate->label); ?></td>
                        <td class="px-4 py-3.5 text-sm text-gray-800 dark:text-white/90">$<?php echo e(number_format($shippingRate->base_rate, 2)); ?></td>
                        <td class="px-4 py-3.5 text-sm text-gray-500 dark:text-gray-400">
                            <?php echo e($shippingRate->free_threshold ? '$'.number_format($shippingRate->free_threshold, 2) : '—'); ?>

                        </td>
                        <td class="px-4 py-3.5 text-sm text-center text-gray-500 dark:text-gray-400"><?php echo e($shippingRate->priority); ?></td>
                        <td class="px-4 py-3.5 text-sm text-center">
                            <?php if($shippingRate->is_active): ?>
                        ...
                        <td class="px-4 py-3.5 text-sm text-center">
                            <div class="flex items-center justify-center space-x-3">
                                <a href="<?php echo e(route('admin.shipping-rates.edit', $shippingRate)); ?>"
                                   class="text-brand-500 hover:text-brand-600 font-medium">Edit</a>
                                <form action="<?php echo e(route('admin.shipping-rates.destroy', $shippingRate)); ?>" method="POST"
                                      onsubmit="return confirm('Delete this shipping rate?')" class="inline">
                ...
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                    <tr><td colspan="8" class="px-4 py-10 text-center text-sm text-gray-400 dark:text-gray-500">No shipping rates found.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
</div>
<div class="mt-4"><?php echo e($shippingRates->links()); ?></div>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('admin.layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\jdira\Herd\beautyshop\BeautyBackend\resources\views/admin/shipping-rates/index.blade.php ENDPATH**/ ?>