...
@section('content')
<x-common.page-breadcrumb pageTitle="Shipping Rates" />
            ...
            <tbody>
                @forelse($shippingRates as $shippingRate)
                    <tr class="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-white/5">
                        <td class="px-4 py-3.5 text-sm font-medium text-gray-800 dark:text-white/90">{{ $shippingRate->country }}</td>
                        <td class="px-4 py-3.5 text-sm text-gray-500 dark:text-gray-400">{{ $shippingRate->region ?? '—' }}</td>
                        <td class="px-4 py-3.5 text-sm text-gray-500 dark:text-gray-400">{{ $shippingRate->label }}</td>
                        <td class="px-4 py-3.5 text-sm text-gray-800 dark:text-white/90">${{ number_format($shippingRate->base_rate, 2) }}</td>
                        <td class="px-4 py-3.5 text-sm text-gray-500 dark:text-gray-400">
                            {{ $shippingRate->free_threshold ? '$'.number_format($shippingRate->free_threshold, 2) : '—' }}
                        </td>
                        <td class="px-4 py-3.5 text-sm text-center text-gray-500 dark:text-gray-400">{{ $shippingRate->priority }}</td>
                        ...
                        <td class="px-4 py-3.5 text-sm text-center">
                            <div class="flex items-center justify-center space-x-3">
                                <a href="{{ route('admin.shipping-rates.edit', $shippingRate) }}"
                                   class="text-brand-500 hover:text-brand-600 font-medium">Edit</a>
                                <form action="{{ route('admin.shipping-rates.destroy', $shippingRate) }}" method="POST"
...
<div class="mt-4">{{ $shippingRates->links() }}</div>
@endsection
